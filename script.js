// File Version: v82
// Last Updated: 2025-06-26 (Fix TypeError, Enhanced Sorting Debugging)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateMainButtonsState(enable) {
        // These buttons are now in the mobile menu, but we enable/disable them based on overall app state
    ...
    function renderWatchlistSelect() {
        if (!watchlistSelect) {
            console.warn("[renderWatchlistSelect] watchlistSelect element not found.");
            return;
        }
        ...
        if (userWatchlists.length === 0) {
            ...
            if (manageWatchlistsBtn) { // Add null check
        ...
        if (manageWatchlistsBtn) { // Add null check
            manageWatchlistsBtn.disabled = false; // Enabled if watchlists exist
        }
    }
    ...
    function renderSortSelect() {
        if (!sortSelect) {
            console.warn("[renderSortSelect] sortSelect element not found.");
            return;
        }
        ...
        if (sortSelect.options.length === 0 || sortSelect.options[0].value !== '') {
            const placeholderOption = document.createElement('option');
            placeholderOption.value = '';
            placeholderOption.textContent = 'Sort'; // Text for the placeholder
            placeholderOption.disabled = true;
            placeholderOption.selected = true; // Make it selected by default
    ...
    if (manageWatchlistsBtn) {
        manageWatchlistsBtn.addEventListener('click', () => {
            ...
            showCustomAlert("Manage Watchlists functionality coming soon! This will open a new modal for adding/renaming/deleting watchlists.");
    ...
    function renderAsxCodeButtons() {
        if (!asxCodeButtonsContainer) {
            console.warn("[renderAsxCodeButtons] asxCodeButtonsContainer element not found.");
            return;
        }
    ...
    function scrollToShare(asxCode) {
        ...
        if (targetShare) {
            ...
            if (!elementToScrollTo && window.matchMedia("(max-width: 768px)").matches) { // Consistent breakpoint with CSS
                elementToScrollTo = document.querySelector(`.mobile-card[data-doc-id="${targetShare.id}"]`); // Changed class name
            }
    ...
    function renderWatchlist() {
        ...
        sharesToRender.forEach((share) => {
            ...
            if (window.matchMedia("(max-width: 768px)").matches) { // Consistent breakpoint with CSS
    ...
    function clearWatchlistUI() {
        ...
        if (watchlistSelect) watchlistSelect.disabled = true;
        if (manageWatchlistsBtn) { // Null check added
             manageWatchlistsBtn.disabled = true;
        }
    ...
    function sortShares() {
        console.log("[Sort] Entering sortShares function."); // NEW Debug log
        const sortValue = sortSelect.value;
        // If the placeholder is selected, default to a sorting or do nothing.
        if (!sortValue || sortValue === '') {
            console.log("[Sort] Sort By placeholder selected, no explicit sorting applied.");
        ...
        allSharesData.sort((a, b) => {
            ...
            if (field === 'lastFetchedPrice' || field === 'dividendAmount' || field === 'currentPrice' || field === 'targetPrice' || field === 'frankingCredits') {
                ...
                // If one is empty and the other is not, place empty at the end for both asc/desc (consistent)
                if (nameA === '') return 1; // Empty 'A' comes after non-empty 'B'
                if (nameB === '') return -1; // Empty 'B' comes before non-empty 'A'
    ...
    if (newShareBtn) {
        newShareBtn.addEventListener('click', () => {
            ...
            if (mobileMenu && mobileMenu.classList.contains('open')) { // Added null check for mobileMenu
                toggleMobileMenu();
            }
        });
    }
    ...
    function showShareDetails() {
        ...
        if (share.comments && Array.isArray(share.comments) && share.comments.length > 0) {
            ...
            modalCommentsContainer.innerHTML = '<p style="text-align: center; color: var(--label-color);">No comments for this share.</p>'; // Used CSS variable
        }
    ...
    function applySavedTheme() {
        ...
        if (savedTheme) {
            ...
            console.log(`[Theme] Applied saved theme from localStorage: ${savedTheme}`);
        } else {
            // If no theme is saved, apply system default
            if (systemPrefersDark) {
                ...
                body.classList.remove('dark-theme');
                if (themeToggleBtn) themeToggleBtn.innerHTML = '<i class="fas fa-moon"></i> Toggle Theme';
                localStorage.setItem('theme', 'light'); // Save system preference as default
    ...
    if (scrollToTopBtn) {
        ...
        window.addEventListener('scroll', () => {
            ...
            // Using 768px as breakpoint as per CSS
    ...
    if (hamburgerBtn && mobileMenu && closeMenuBtn && menuOverlay) {
        ...
        // Close menu if a menu item is clicked (assumes menu items are buttons that trigger modals/actions)
        mobileMenu.querySelectorAll('button').forEach(button => {
            // Exclude the close button itself from this listener
            if (button.id !== 'closeMenuBtn') {
                button.addEventListener('click', () => {
                    // Only close menu if it's currently open AND the click isn't the close button
    ...
    function toggleMobileMenu() {
        if (mobileMenu && menuOverlay) { // Add null checks
