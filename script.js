...
<head>
    ...
    <script type="module">
        console.log("INDEX.HTML: Module script started."); // LOG 1: Confirm script begins
        ...
        console.log("INDEX.HTML: Firebase SDKs imported."); // LOG 2: Confirm imports succeed
        ...
        let app, db, auth; // Declare these as `let` here to allow assignment within try/catch

        try {
            console.log("INDEX.HTML: Attempting initializeApp..."); // LOG 3: Before initializeApp
            app = initializeApp(firebaseConfig);
            console.log("INDEX.HTML: initializeApp successful. Attempting getFirestore..."); // LOG 4: After initializeApp
            db = getFirestore(app);
            console.log("INDEX.HTML: getFirestore successful. Attempting getAuth..."); // LOG 5: After getFirestore
            auth = getAuth(app);
            console.log("INDEX.HTML: getAuth successful. Exposing globals..."); // LOG 6: After getAuth
            ...
            // For the Canvas environment, handle initial custom token sign-in
            const initialAuthToken = typeof __initial_auth_token !== 'undefined' ? __initial_auth_token : null;
            if (initialAuthToken) {
                signInWithCustomToken(auth, initialAuthToken)
                    .then(() => console.log("Firebase Init: Signed in with custom token (Canvas)."))
                    .catch(customTokenError => console.warn("Firebase Init: Custom token sign-in failed:", customTokenError.message, "script.js will handle other auth paths."));
            }
            console.log("Firebase Init: All services initialized and exposed globally."); // LOG 7: Final success log

        } catch (error) {
            console.error("Firebase Init ERROR: Failed to initialize Firebase services. This is critical.", error); // LOG 8: Critical error catch
            window.firebaseInitError = true;
            window.firebaseErrorMessage = error.message;

            const loadingIndicator = document.getElementById('loadingIndicator');
            if (loadingIndicator) {
                loadingIndicator.textContent = `CRITICAL ERROR: Failed to load app. Check console. Details: ${error.message}`;
            }
        }
    </script>
</head>
