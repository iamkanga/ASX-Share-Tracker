...
document.addEventListener('DOMContentLoaded', function() {
    console.log("script.js (v74) DOMContentLoaded fired."); // New log to confirm script version and DOM ready
    ...
    if (googleAuthBtn) {
        googleAuthBtn.addEventListener('click', async () => {
            ...
            if (!auth) { // Use the 'auth' variable from the script.js scope
                ...
                showCustomAlert("Authentication service not ready. Please check Firebase configuration in index.html."); // Changed alert message
                return;
            }
    ...
    if (addWatchlistBtn) {
        addWatchlistBtn.addEventListener('click', async () => {
            if (!currentUserId || !window.firestore) {
                showCustomAlert("Please sign in to add a watchlist, or Firebase is not initialized.");
                return;
            }
    ...
    if (renameWatchlistBtn) {
        renameWatchlistBtn.addEventListener('click', async () => {
            ...
            if (!currentWatchlistId || !currentUserId || !window.firestore) {
                showCustomAlert("Please select a watchlist to rename, sign in, or Firebase is not initialized.");
                return;
            }
