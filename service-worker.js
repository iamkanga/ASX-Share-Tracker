...
const CACHE_NAME = 'asx-tracker-v18'; 
...
self.addEventListener('install', (event) => {
    console.log('Service Worker v18: Installing...'); // Updated log for version
    event.waitUntil(
        caches.open(CACHE_NAME)
            .then((cache) => {
                console.log('Service Worker v18: Cache opened'); // Updated log for version
            ...
            .then(() => {
                ...
                self.skipWaiting();
                console.log('Service Worker v18: Installation complete and skipWaiting called.'); // Updated log
            })
            .catch((error) => {
                console.error('Service Worker v18: Cache addAll failed during install:', error); // Updated log
            })
    );
});

self.addEventListener('activate', (event) => {
    console.log('Service Worker v18: Activating...'); // Updated log for version
    event.waitUntil(
        caches.keys().then((cacheNames) => {
            return Promise.all(
                cacheNames.map((cacheName) => {
                    if (cacheName !== CACHE_NAME) {
                        console.log(`Service Worker v18: Deleting old cache: ${cacheName}`); // Updated log
            ...
            console.log('Service Worker v18: Old caches cleared, claiming clients.'); // Updated log
        ...
        event.respondWith(
            caches.match(event.request).then((cachedResponse) => {
                ...
                const fetchPromise = fetch(event.request).then(response => {
                    ...
                    console.error(`Service Worker v18: Network fetch failed for ${event.request.url}.`, error); // Updated log
                ...
                console.error(`Service Worker v18: Cache match failed for ${event.request.url}.`, error); // Updated log
...
self.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'SKIP_WAITING') {
        self.skipWaiting();
        console.log('Service Worker v18: Skip waiting message received, new SW activated.'); // Updated log
    }
});
