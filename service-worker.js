...
const CACHE_NAME = 'asx-tracker-v6'; // New cache name for cache busting
const CACHED_ASSETS = [
    '/', // Cache index.html
    '/index.html',
...
self.addEventListener('install', (event) => {
    ...
    event.waitUntil(
        caches.open(CACHE_NAME)
            .then((cache) => {
                console.log('Service Worker v6: Cache opened');
                return cache.addAll(CACHED_ASSETS);
            })
            .then(() => self.skipWaiting()) // Activates the new service worker immediately
            .catch((error) => console.error('Service Worker v6: Cache.addAll failed', error))
    );
});
...
self.addEventListener('fetch', (event) => {
    ...
    if (event.request.mode === 'navigate' || event.request.url.includes('script.js') || event.request.url.includes('style.css')) {
        console.log(`Service Worker v6: Fetching ${event.request.url} (Network First)`);
        event.respondWith(
            fetch(event.request)
                .then(response => {
                ...
                .catch(async (error) => {
                    console.error(`Service Worker v6: Network fetch failed for ${event.request.url}. Trying cache.`, error);
        ...
        event.respondWith(
            caches.match(event.request).then((cachedResponse) => {
                ...
                return fetch(event.request).then(response => {
                    ...
                    console.error(`Service Worker v6: Network fetch failed for ${event.request.url}.`, error);
...
self.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'SKIP_WAITING') {
        self.skipWaiting();
        console.log('Service Worker v6: Skip waiting message received, new SW activated.');
    }
});
